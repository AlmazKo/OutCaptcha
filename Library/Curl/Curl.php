...
class Curl implements \Library\OutCaptcha\FileDownloader {
    ...
    protected static $_defaultEncode = 'utf-8';
     ...
     * @return bool
     */
    public function downloadFile($url, $path, $fileExtension = '') {
        $this->_clean();
        $this->_preparePost();
    ...
    protected function _preparePost($prepare = true) {
        if ($this->getOpt(CURLOPT_POST)) {
            ...
            if (ConstCurl::FORM_ENCTYPE_APP == $this->_formEnctype) {
                ...
                foreach ($this->_aPost as $key => $value) {
                    $post[$key] = @iconv(self::$_defaultEncode, $this->_responseCharset, $value);
                }
...
//Log::dump($aHeaders);
    ...
    protected function _parseResponse($convert = true) {
        ...
        if (!$this->isOpt(CURLOPT_NOBODY)) {
            ...
            if ($convert && $charset && self::$_defaultEncode != $charset) {
                $this->_responseBody = @iconv($charset, self::$_defaultEncode, $this->_responseBody);
            }
        }
